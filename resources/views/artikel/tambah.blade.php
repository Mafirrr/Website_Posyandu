...
<div class="">
    ...
    <div style="margin-top:10px;">
        @if($berita->gambar && file_exists(storage_path('app/public/' . $berita->gambar)))
            <img id="gambar-preview" src="{{ asset('storage/' . $berita->gambar) }}" alt="Preview Gambar" style="max-width: 300px; display: block;">
        @else
...
<div class="">
    ...
    <input type="hidden" name="slug" id="slug_hidden" value="{{ old('slug', $berita->slug) }}">
...
<script>
    document.getElementById('judul').addEventListener('input', function () {
        ...
        document.getElementById('slug_hidden').value = slug;
                            ...
                            <div class="">
                                ...
                                <textarea id="isi" name="isi" rows="4" placeholder="Deskripsi Edukasi" class="form-control @error('isi') is-invalid @enderror">{{ old('isi', $berita->isi) }}</textarea>
                                @error('isi')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
    ...
    <div class="col-md-6 col-12">
        ...
        <select id="kategori_edukasi" name="kategori_edukasi" class="form-control">
            ...
            <option value="kesehatan" {{ old('kategori_edukasi', $berita->kategori_edukasi ?? '') == 'kesehatan' ? 'selected' : '' }}>kesehatan</option>
